...
module.exports.buildTestcaseTable = function (testcasesFile, inputDeserializer = JSON.parse) {
    let prevLines = 0
    let blocks = fs.readFileSync(testcasesFile, 'utf-8').split(/(?:\r?\n){2}/)
    const [ inputs, expects ] = blocks.reduce((cases, block, blockno) => {
        ...
        if (blockno === 0 || blockno === blocks.length - 1) {
            ...
            if (blockno === 0) {
                blockLines = blockLines.map(l => inputDeserializer(l))
            }
            cases.push(blockLines)
        }
        prevLines += lines.length + 1 // blank line
        return cases
    }, [])
    
    if (inputs.length % expects.length !== 0) {
        throw new Error('Mismatch between number of testcases and number of expected values')
    }
